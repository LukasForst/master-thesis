...
% TODO: ensure that this appears on the left side and the next subsubsection on the right side
% \clearpage % check if this appears on the left side 
\begin{figure}[hp]
    \centering
...
The target detection performance $tdp$ (Section~\ref{subsec:target-detection-performance-metric}) is the most important metric because it evaluates how good is Fides in the target classification.
...
\begin{figure}[hp]
    \centering
    \includegraphics[height=0.95\textheight]{assets/25_peer_trust.png}
    \caption{Target detection possibility.}
    \label{fig:25-peer-trust}
\end{figure}

\subsubsection{Peers Behavior Detection Performance}
Figure~\ref{fig:25-peer-trust} displays two important metrics which are related to 
