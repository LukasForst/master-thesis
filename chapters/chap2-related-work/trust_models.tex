...
However, we can not use SORT directly especially because of the recommendation algorithm, that can in some cases request a recommendation even from the peers that are not \textit{trusted enough}. 
